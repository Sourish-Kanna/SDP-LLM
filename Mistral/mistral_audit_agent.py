...
{{ audit_json }}"""
    ...
    def audit(self, invoice_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            response = self.chain.invoke({"audit_json": json.dumps(audit_json)})
            cleaned_response = response.content.strip().replace("```", "").replace("json", "").strip()
            fuzzy = json.loads(cleaned_response)
